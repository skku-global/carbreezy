...
export default function App() {
  ...
  useEffect(() => {
    const id = setInterval(() => setVisitorCount(v => v + Math.floor(Math.random() * 3)), 4000)
  ...
  return (
    <>
      <Header view={view} goto={goto} visitorCount={visitorCount} menuOpen={menuOpen} setMenuOpen={setMenuOpen} />

      {view === 'home' && <Home goto={goto} openModal={openModal} showToast={showToast} />}
      {view === 'new' && <Listing condition="new" title="New Cars" tag="BRAND NEW STOCK" blurb="Zero-mileage vehicles, fresh off the lot, with full manufacturer warranty." openModal={openModal} />}
      {view === 'used' && <Listing condition="used" title="Used Cars" tag="CERTIFIED PRE-OWNED" blurb="Inspected, verified, and priced fairly — every used car comes with a condition report." openModal={openModal} />}
